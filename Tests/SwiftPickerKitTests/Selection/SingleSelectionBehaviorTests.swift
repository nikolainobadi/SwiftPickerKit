//
//  SingleSelectionBehaviorTests.swift
//  SwiftPickerKit
//
...
struct SingleSelectionBehaviorTests {
    ...
    func emptyStartingValues() {
        ...
        #expect(state.options[0].isSelected == false)

        let backspaceResult = sut.handleSpecialChar(char: .backspace, state: state)
        let spaceResult = sut.handleSpecialChar(char: .space, state: state)

        if case .continueLoop = backspaceResult {} else {
            Issue.record("Expected continueLoop on backspace")
        }
        if case .continueLoop = spaceResult {} else {
...
private extension SingleSelectionBehaviorTests {
    func makeSUT(items: [TestItem] = [TestFactory.makeItem(name: "Item")], activeIndex: Int = 0, fileID: String = #fileID, filePath: String = #filePath, line: Int = #line, column: Int = #column) -> (SingleSelectionBehavior<TestItem>, SelectionState<TestItem>) {
        ...
        let sut = SingleSelectionBehavior<TestItem>()
        return (sut, state)
    }
}
